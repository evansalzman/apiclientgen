...
const moduleTemplate = fs_1.default.readFileSync('./data/templates/moduleTemplateApiClient.mustache', 'utf8');
const functionTemplateGET = fs_1.default.readFileSync('./data/templates/functionTemplateGET.mustache', 'utf8');
const functionTemplateDELETE = fs_1.default.readFileSync('./data/templates/functionTemplateDELETE.mustache', 'utf8');
const functionTemplatePOST = fs_1.default.readFileSync('./data/templates/functionTemplatePOST.mustache', 'utf8');
const functionTemplatePUT = fs_1.default.readFileSync('./data/templates/functionTemplatePUT.mustache', 'utf8');
const interfaceTemplate = fs_1.default.readFileSync('./data/templates/interfaceTemplate.mustache', 'utf8');
const urlParamsConstructorTemplate = fs_1.default.readFileSync('./data/templates/urlParamConstructorTemplate.mustache', 'utf8');
/**
 ...
 */
function Write(swaggerJsonUrl, apiClientName, swaggerCallback, moduleCallback) {
    return __awaiter(this, void 0, void 0, function* () {
        const swaggerJson = yield LoadSwaggerJson(swaggerJsonUrl);
        const swaggerUpdated = swaggerCallback ? swaggerCallback(swaggerJson) : swaggerJson;
        const functionsString = yield GenerateClientCalls(swaggerUpdated, apiClientName); // TODO: make the client lib name a passed in param
        const interfaceString = yield GenerateInterfaces(swaggerUpdated);
        const moduleString = yield GenerateModule(functionsString, interfaceString, apiClientName);
 ...
 * @param apiClientName
 */
function SetupAsNpmProject(apiClientName) {
    const errorCode = 0;
    ...
    if (!fs_1.default.existsSync(`./out/${apiClientName}/.vscode`)) {
        fs_1.default.mkdirSync(`./out/${apiClientName}/.vscode`);
    }
    ...
    let packageJson = fs_1.default.readFileSync(`./out/${apiClientName}/package.json`, 'utf-8');
    ...
    fs_1.default.writeFileSync(`./out/${apiClientName}/package.json`, packageJson);
    return errorCode;
}
/**
 ...
 */
function LoadSwaggerJson(url) {
    return __awaiter(this, void 0, void 0, function* () {
        const logMessagePrefix = 'apiclientwriter.LoadSwaggerJson() ';
        if (url.toLowerCase().includes('http')) {
            return yield requestPromise.get(url)
                .then((response) => {
            ...
            const swaggerFileJson = JSON.parse(fs_1.default.readFileSync(url, 'utf-8'));
            return swaggerFileJson;
        }
    });
}
exports.LoadSwaggerJson = LoadSwaggerJson;
function GenerateClientCalls(swaggerJson, clientLibraryName) {
    return __awaiter(this, void 0, void 0, function* () {
...
function CreateGetFunction(path, apiRequestDefinition, clientLibraryName) {
    ...
    if (urlParamsArray.length > 0) {
        ...
        urlParamsConstructor = mustache_1.default.render(urlParamsConstructorTemplate, uptInputs);
    }
    ...
    const functionString = mustache_1.default.render(functionTemplateGET, templateInputs);
    return functionString;
}
function CreateDeleteFunction(path, apiRequestDefinition, clientLibraryName) {
    const logMessagePrefix = 'apiclientwriter.CreateGetFunction() ';
    ...
    const functionString = mustache_1.default.render(functionTemplateDELETE, templateInputs);
    return functionString;
}
function CreatePutFunction(path, apiRequestDefinition, clientLibraryName, swaggerJson) {
    ...
    // remove the comma from the last entry
    headersCustom[headersCustom.length - 1] = headersCustom[headersCustom.length - 1].slice(0, headersCustom[headersCustom.length - 1].length - 1);
    const formDataTs = GenerateFormDataArray(apiRequestDefinition);
    let bodyOrFormField = `body: ''`;
    if (formDataTs.length > 0) {
        bodyOrFormField = 'form';
    }
    if (parameterSignature.search('body:') > 1) {
        bodyOrFormField = 'body';
    }
    ...
    const functionString = mustache_1.default.render(functionTemplatePUT, templateInputs);
    return functionString;
}
function CreatePostFunction(path, apiRequestDefinition, clientLibraryName, swaggerJson) {
    ...
    const functionString = mustache_1.default.render(functionTemplatePOST, templateInputs);
    return functionString;
}
 ...
 */
function GenerateUrlParamsArray(apiRequestDefinition) {
...
function GenerateFormDataArray(apiRequestDefinition) {
    const formData = [];
...
function GenerateAcceptHeader(apiRequestDefinition, swaggerJson) {
...
function GenerateContentTypeHeader(apiRequestDefinition, swaggerJson) {
...
function GenerateFunctionDocumentation(getDef) {
...
function GenerateCustomHeaderEntries(requestDefinition) {
    const customHeaders = [];
...
function GenerateFunctionParameterSignature(apiRequestDefinition) {
...
function GenerateInterfaces(swaggerJson) {
    ...
    Object.entries(swaggerJson.definitions).forEach((entry) => {
        ...
        if (currDef.hasOwnProperty('properties')) {
            Object.keys(currDef.properties).forEach((propertyName) => {
                ...
                const propDescription = currDef.properties[propertyName].description;
...
function GetPropertyType(currDef, propertyName) {
    let propType;
 ...
 * @param currDef
 */
function GetArrayTypeOfInterfaceProperty(propertyName, currDef) {
    let propType = 'any[]';
...
function GenerateInterfaceString(interfaceName, interfaceProperties, interfaceDocumentation) {
    let interfaceString;
    const templateInputs = {
    ...
    interfaceString = mustache_1.default.render(interfaceTemplate, templateInputs);
    return interfaceString;
}
function GenerateModule(functionsString, interfaceString, apiClientName) {
    const templateInputs = {
        apiClientName,
        functions: functionsString,
        interfaces: interfaceString
    };
    const moduleString = mustache_1.default.render(moduleTemplate, templateInputs);
    return moduleString;
}
exports.GenerateModule = GenerateModule;
function WriteModule(moduleString, apiClientName) {
    let errorValue = 0;
    ...
    try {
        ...
        if (!fs_1.default.existsSync(`./out/${apiClientName}`)) {
            fs_1.default.mkdirSync(`./out/${apiClientName}`);
        }
        if (!fs_1.default.existsSync(`./out/${apiClientName}/src`)) {
            fs_1.default.mkdirSync(`./out/${apiClientName}/src`);
        }
        fs_1.default.writeFileSync(`./out/${apiClientName}/src/${apiClientName}.ts`, moduleString);
    }
    catch (e) {
        console.error(e);
        errorValue = 1;
...
function CleanVariableNames(varString) {
    return varString ? varString.replace(/[-_\ ]/g, '') : varString;
}
function CleanInterfaceNames(varString) {
    ...
    const interfaceName = `I${cleanedInterfaceName}`;
    return interfaceName;
}
//# sourceMappingURL=apiclientwriter.js.map